...
if (!fs.existsSync(directorioPDFS)) {
  fs.mkdirSync(directorioPDFS);
}
var config = leerJsonConfig();

function leerJsonConfig() {
  try {
...
let info = "";
info += `<p>Asegúrate de que la carpeta <b>${directorioPDFS}</b> esté vacía antes de empezar.</p>`;
info += "<br />";
...
function mostrarDia() {
  ...
  let show = info;
  show +=
    "<p>Tras elegir el día en el selector superior, deja caer aquí los albaranes y facturas que te falten, de uno en uno.</p><br />";
  ...
  const noEnDocumentos = listadoCarpeta.filter(
    (doc) => !documentos.includes(doc)
  );
  if (noEnDocumentos.length != 0) {
    alert(
      `Tienes documentos pdf en la carpeta ${directorioPDFS} que no están en el listado.`
    );
  }
...
async function combinarPDFsConA() {
  try {
    ...
    const downloadPath = path.resolve(
      finalDownloadPath,
      "ALBARANES-" + fechaCarga + ".pdf"
    );

    // Filtrar archivos que terminan en "A.pdf"
    const archivos = fs.readdirSync(directorioPDFS);
    const pdfsFiltrados = archivos.filter((archivo) =>
      archivo.endsWith("A.pdf")
    );
    ...
    for (const pdfNombre of pdfsFiltrados) {
      ...
      for (const [index, page] of pdfCargado.getPages().entries()) {
        ...
        let numAlmacen = "";
        if (config["checkboxes"].albaran_almacen)
          numAlmacen = await obtenerTexto(1, pdfNombre);
        let textoObservaciones = "";
        if (config["checkboxes"].albaran_observaciones1)
          textoObservaciones = await obtenerTexto(2, pdfNombre);
    ...
    alert(`PDF de ALBARANES combinado guardado en: ${downloadPath}`);
...
async function combinarPDFsConF() {
  try {
    ...
    const downloadPath = path.resolve(
      finalDownloadPath,
      "FACTURAS-" + fechaCarga + ".pdf"
    );

    // Filtrar archivos que terminan en "F.pdf"
    const archivos = fs.readdirSync(directorioPDFS);
    const pdfsFiltrados = archivos.filter((archivo) =>
      archivo.endsWith("F.pdf")
    );
    ...
    for (const pdfNombre of pdfsFiltrados) {
      ...
      if (totalPaginas >= 3 && config["checkboxes"].facturas_pag_2) {
    ...
    const pdfBytesFinal = await pdfFinal.save();
    fs.writeFileSync(downloadPath, pdfBytesFinal);
    alert(`PDF de FACTURAS combinado guardado en: ${downloadPath}`);
    console.log(`PDF combinado guardado en: ${downloadPath}`);
  } catch (error) {
    console.error("Error al combinar los PDFs:", error);
  }
...
function probarSiContiene(documento, ruta) {
  console.log("Comprobando el pdf subido...");
        ...
        cuenta.pedidos.forEach((pedido) => {
          if (documento.includes(pedido.pedido.toUpperCase())) {
            ...
            if (documento.includes(pedido.lineas[0].codigo.toUpperCase())) {
              // ES DEL PEDIDO QUE ESTAMOS COMPROBANDO PORQUE EL ARTÍCULO COINCIDE
              ...
              if (documento.includes("EFAC")) {
                ...
                moverArchivo(
                  ruta,
                  `${carga.numero}-${pedido.pedido}-F.pdf`,
                  "FACTURA",
                  cuenta.nombre
                );
                return true;
                ...
                moverArchivo(
                  ruta,
                  `${carga.numero}-${pedido.pedido}-A.pdf`,
                  "ALBARAN",
                  cuenta.nombre
                );
                return true;
              } else if (documento.includes("PEDIDO DE DEVO")) {
                // ES UN RMA
                console.log(`Nombre RMA: ${carga.numero}-${pedido.pedido}-A.pdf`);
                moverArchivo(
                  ruta,
                  `${carga.numero}-${pedido.pedido}-A.pdf`,
                  "RMA",
                ...
                // NO SE QUÉ ES
                console.log("No se si es un albarán, una factura o un RMA.");
                alertaSubida(false);
                return false;
              }
            }
            ...
            // ES UN RMA
            console.log("Comprobando si es un pedido de devolución...");
            if (documento.includes(pedido.pedido.toUpperCase())) {
              // ES UN RMA DE ESTE CLIENTE
              console.log(`Nombre RMA: ${carga.numero}-${pedido.pedido}-A.pdf`);
              moverArchivo(
                ruta,
                `${carga.numero}-${pedido.pedido}-A.pdf`,
              ...
              console.log("Es un RMA, pero no identifico el cliente por el número de cuenta.");
            }
          } else {
...
setInterval(function () {
  console.log(
    "Rellamada a cargar Json de cargas y mostrar listado actualizado"
  );
  leerDia();
  mostrarDia();
}, 5000);
...
async function changeBackground(auxBol) {
  if (auxBol) displayMessage.style.backgroundImage = `url("./img/rep.png")`;
    ...
    displayMessage.style.backgroundImage = `url("./img/cancel.png")`;
}
